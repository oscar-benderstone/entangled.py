...
def read_code_file(path: Path, refs: ReferenceMap, hooks: list[HookBase] | None = None):
    with open(path, "r") as f:
        read_code_string(f.read(), path, refs, hooks)


def read_code_string(
    text: str,
    path: Path,
    refs: ReferenceMap,
    hooks: list[HookBase] | None = None,
):
    code = CodeReader(path, refs)
    code.run(text)

    hooks = hooks if hooks is not None else []

    # TODO: update raw content from
